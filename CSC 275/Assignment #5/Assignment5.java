...
public class Assignment5 {
	...
	public Assignment5() {
		...
		while (true) {
			...
			case 7:
				...
				ransack(fileName, ran_sack, item, size);
				System.out.println("That is what we can have based on our current weight of " + sum);
				System.out.println();
				break;
			case 0:
	...
	private void addItem(ArrayList<Item> cargohold, ArrayList<Integer> arrayID, ArrayList<Double> totalWeight) {
		...
		if (choice.charAt(0) == 'Y') {
			...
			File outFile = new File("ransack.txt");
			try {
...
// NOTE: This method does not delete the item from the input file. Only from the current buffer. 
	...
	private void sortItems(ArrayList<Item> cargohold) { // NOTE: works just fine, but it is hard to see because the
														// display
														// method only outputs one subclass at a time (you can still see
														// how indices change in display method)
	...
	private void displayItems(ArrayList<Item> cargohold) { // NOTE: index at the beginning of each line might change
		...
		for (int j = 0; j < cargohold.size(); j++) {
			if (cargohold.get(j) instanceof Equippable)
				System.out.printf("%-3s%-200s\n", (cargohold.indexOf(cargohold.get(j)) + 1), cargohold.get(j));
		}
		for (int j = 0; j < cargohold.size(); j++) {
	...
	public void ransack(String fileName, Double ran_sack, ArrayList<Integer> item, int size) {
		String nameRS = null;
		...
		try {
			...
			int counter = 0;
			int temp = 0;
			if (ran_sack >= 25000 || item.size() == size) {
				return;
			} else {
				while (fileInput.hasNext()) {
					...
					if (weight < min && !item.contains(counter)) {
						min = weight;
						choice0 = choice1;
						temp = counter;
						nameRS = name;
				...
				if ((ran_sack + min <= 25000)) {
					...
					ransack(fileName, ran_sack, item, size); // NOTE: Recursion
